...
struct Camera
{
    ...
    void orbitY(float degrees, const Vector3 &origin = {0, 0, 0})
    {
        float radians = degrees * PI / 180.0f;

        Vector3 relPos = position - origin;

        float cosA = std::cos(radians);
        float sinA = std::sin(radians);

        float newX = relPos.x * cosA - relPos.z * sinA;
        float newZ = relPos.x * sinA + relPos.z * cosA;

        position = {newX + origin.x, relPos.y + origin.y, newZ + origin.z};

        direction = (origin - position).normalize();
    }
};
...
void renderScene(const std::string &in, const std::string &out, const Camera &cam)
{
    std::vector<Triangle> tris;
    std::vector<Material> mats;
    std::vector<Light> lights;
    Vector3 camPos, bg, right, up, forward;
    AABB sceneBox;
    int w, h;
    if (!loadScene(in, tris, mats, camPos, bg, w, h, right, up, forward, lights, sceneBox))
    {
    ...
    camPos = cam.position;
    forward = cam.direction.normalize();
        ...
        while (true)
        {
            ...
            for (int y = bucket.second; y < std::min(h, bucket.second + BUCKET_SIZE); ++y)
            {
                for (int x = bucket.first; x < std::min(w, bucket.first + BUCKET_SIZE); ++x)
                {
                    ...
                    framebuffer[y][x] = traceRay(cam.position, dir, kdRoot, lights, bg, sceneBox, 0);
                }
            }
        }
    };
...
int main()
{
    ...
    for (int i = 0; i < 72; ++i)
    {
        ...
        filename << "frame_" << std::setw(2) << std::setfill('0') << i << ".ppm";

        renderScene("scene0.crtscene", filename.str(), cam);
    }
    return 0;
}