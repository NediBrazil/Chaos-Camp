...
Vector3 traceRay(const Vector3 &orig, const Vector3 &dir,
                 ...
                 const Vector3 &bg, const AABB &sceneBox, int depth = 0)
{
    ...
    if (hit->material.type == REFLECTIVE)
    {
        ...
        Vector3 rc = traceRay(P + N * 0.001f, rd, trs, lights, bg, sceneBox, depth + 1);
        ...
        while (true)
        {
            ...
            for (int y = bucket.second; y < std::min(h, bucket.second + BUCKET_SIZE); ++y)
            {
                for (int x = bucket.first; x < std::min(w, bucket.first + BUCKET_SIZE); ++x)
                {
                    ...
                    framebuffer[y][x] = traceRay(cam, dir, tris, lights, bg, sceneBox, 0);
                }
            }
        }
    };
